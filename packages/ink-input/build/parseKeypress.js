...
export function setKittyProtocolEnabled(enabled) {
...
function parseKittyProtocol(data) {
...
export const parseKeypress = (data) => {
    // Always try Kitty CSI-u parsing first — the format (ESC [ keycode ; modifiers u)
    // is unambiguous and some terminals send it even without explicit protocol negotiation.
    // The kittyProtocolEnabled flag controls whether we SEND the enable sequence,
    // not whether we can PARSE the responses.
    ...
    if (s === "\r") {
        ...
        // ESC + CR = Alt+Enter (meta+return)
        // Ink's useInput strips the ESC prefix and delivers input='\r', key.return=false.
        // We match that behavior: return raw '\r' so handlers can detect (input === '\r' && !key.return).
        return { input: "\r", key };
    }
    else if (s === " " || s === "\x1b ") {
        key.meta = s.length === 2;
...
/\x1b(?:\[[0-9:;<>?]*[A-Za-z~^$Mm]|\]8;[^\x1b]*(?:\x07|\x1b\\)?|O[A-Za-z]|[a-zA-Z0-9]|\x1b|\r|\x7f|\b| )/g;
export function splitInputChunks(data) {
...
function isIncompleteEscapeChunk(chunk) {
    ...
    if (chunk.startsWith("\x1b[")) {
        ...
        if (/^\x1b\[[0-9:;]+u$/.test(chunk))
            return false; // Kitty CSI-u
        // biome-ignore lint/suspicious/noControlCharactersInRegex: ESC is intentional in terminal parsing
        if (/^\x1b\[\d+(?:;\d+)?[~^$]$/.test(chunk))
            return false; // fn/edit keys
        // biome-ignore lint/suspicious/noControlCharactersInRegex: ESC is intentional in terminal parsing
        if (/^\x1b\[(?:1;)?\d?[a-zA-Z]$/.test(chunk))
            return false; // arrows/home/end variants
        // biome-ignore lint/suspicious/noControlCharactersInRegex: ESC is intentional in terminal parsing
        if (/^\x1b\[<\d+;\d+;\d+[Mm]$/.test(chunk))
            return false; // SGR mouse
        if (chunk.startsWith("\x1b[200~") || chunk.startsWith("\x1b[201~"))
            return false; // paste markers
        return true;
    }
...
export function splitInputChunksWithRemainder(data) {
...
export function parseMouseEvent(data) {
    ...
    if (hasSgrMouse) {
        ...
        for (const match of data.matchAll(sgrRegex)) {
            ...
            allMatches.push({ match: match, event });
        }
        if (allMatches.length === 0) {
            return { mouse: null, consumed: false };
        }
        // Collect non-mouse gaps between matches
        const unconsumedParts = [];
        let lastEnd = 0;
        ...
        if (wheelUpCount > 0) {
            ...
            mouse = events[events.length - 1] ??
                events[0] ?? { type: "move", button: 0, x: 0, y: 0, count: 1 };
        }
    ...
    if (data.length >= 6 && data.startsWith("\x1b[M")) {
        ...
        if (rawButton === 64)
            event = { type: "wheel-up", button: 64, x, y, count: 1 };
        else if (rawButton === 65)
            event = { type: "wheel-down", button: 65, x, y, count: 1 };
        else if (rawButton & 32)
            event = { type: "drag", button, x, y };
        else if (rawButton === 3)
            event = { type: "release", button, x, y };
        else
            event = { type: "click", button, x, y };
...
//# sourceMappingURL=parseKeypress.js.map