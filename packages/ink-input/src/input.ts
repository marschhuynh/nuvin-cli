...
const mouseSubscribers = new Map<string, MouseSubscriber>();
let middleware: InputMiddleware[] = [];
let idCounter = 0;
...
let sortedSubscribersCache: Subscriber[] = [];
let sortedMouseSubscribersCache: MouseSubscriber[] = [];
let subscribersDirty = true;
let mouseSubscribersDirty = true;

function clearEscapeFlushTimer(): void {
  if (escapeFlushTimer) {
 ...
 */
function distributeInput(input: string, key: Key): void {
  if (subscribersDirty) {
 ...
 * Stops when a handler returns true.
 */
function distributeMouse(event: MouseEvent): void {
 ...
 */
function dispatchParsedChunk(chunk: string): void {
...
export const inputStore = createStore<InputState & InputActions>()((set, _get) => ({
  // Reactive state (triggers re-renders when changed)
  isMouseModeEnabled: false,

  init: (stdoutStream: NodeJS.WriteStream) => {
    stdout = stdoutStream;
   ...
   */
  subscribe: (handler: InputHandler, options: UseInputOptions = {}) => {
   ...
   */
  subscribeMouse: (handler: MouseHandler, options: UseMouseOptions = {}) => {
  ...
  updateSubscriber: (id: string, options: Partial<UseInputOptions>) => {
  ...
  addMiddleware: (mw: InputMiddleware) => {
    middleware.push(mw);
    return () => {
 ...
 */
export function setMiddleware(mw: InputMiddleware[]): void {
  middleware = [...mw];
}
...
export function useInputStoreSelect<T>(selector: (state: InputState & InputActions) => T): T {
