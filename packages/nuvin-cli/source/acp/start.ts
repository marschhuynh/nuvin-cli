...
export async function startAcpServer({
  ...
  commandRegistry.setConfigFunctions({
    get: <T>(key: string, scope?: ConfigScope) =>
      configManager.get(key, scope) as T | undefined,
    set: (key: string, value: unknown, scope?: ConfigScope) =>
      configManager.set(key, value, scope ?? "auto"),
    delete: (key: string, scope?: ConfigScope) =>
      configManager.delete(key, scope ?? "global"),
  });
  ...
  eventBus.on("custom-command:execute", async (payload) => {
    try {
      if (payload.renderedPrompt) {
        const userPayload = toUserMessagePayload([
          { type: "text", text: payload.renderedPrompt },
        ]);
