...
import { HookEventTypes, HookDecision, type HookResult, type HookContext, type HookPort } from '../../hooks/types.js';
...
describe('Hook Types', () => {
  ...
  it('should define HookPort interface structure', () => {
    const mockPort: HookPort = {
      executeHook: async (_context: HookContext): Promise<HookResult> => ({
        continue: true,
        exitCode: 0,
      }),
      hasHooks: (_event: string, _matcher?: string): boolean => true,
    };
