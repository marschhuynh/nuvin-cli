...
function findSystemRipgrep(): string | null {
  ...
  if (isWindows) {
    ...
    try {
      const result = execSync('where rg', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] })
        .trim()
        .split('\n')[0];
      if (result && existsSync(result)) return result;
    } catch {}
  } else {
    const unixPaths = ['/opt/homebrew/bin/rg', '/usr/local/bin/rg', '/usr/bin/rg', '/bin/rg'];
...
async function extractZip(zipPath: string, destDir: string, rgFilename: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const proc = spawn(
      'powershell',
      ['-NoProfile', '-Command', `Expand-Archive -Path "${zipPath}" -DestinationPath "${tempExtractDir}" -Force`],
      {
        stdio: ['ignore', 'pipe', 'pipe'],
      },
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      for (const line of lines) {
        ...
        if (opts.context && parts.length >= 4 && !Number.isNaN(parseInt(parts[2], 10))) {
