...
import { Box, measureElement, Text } from "@nuvin/ink";
import { useInput } from "@nuvin/ink-input";
import React, { useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
import { moveCursorVertically, moveCursorVisually } from "./textNavigation.js";
...
function TextInput({ value: originalValue, placeholder = "", focus = true, mask, showCursor = true, onChange, onSubmit, vimModeEnabled = false, onVimModeChange, onUpArrow, onDownArrow, onTab, maxLines, showScrollbar = true, scrollbarColor, scrollbarTrackColor, }) {
    ...
    const { mode: vimMode, handleVimInput, enterInsertMode, } = useVimMode({
        enabled: vimModeEnabled,
        onModeChange: onVimModeChange,
    });
    const { state: editorState, stateRef: editorStateRef, setValue, moveCursor, setInitialCursor, } = useEditorState({
    ...
    const scrollRatio = useMemo(() => {
        if (!hasScrolling || visualLineCount <= visibleLines)
            return 0;
    ...
    const getVisualRowStart = useCallback((cursorOffset) => {
    ...
    const getVisualRowEnd = useCallback((cursorOffset) => {
        const lineInfo = lineIndexRef.current.getLineInfo(cursorOffset);
        if (effectiveWidth && effectiveWidth > 0) {
    ...
    const handleInput = useCallback((input, key) => {
        ...
        if (key.leftArrow) {
            ...
            const totalWrappedRows = effectiveWidth && effectiveWidth > 0
                ...
                : 1;
            const currentWrappedRow = effectiveWidth && effectiveWidth > 0 ? Math.floor(lineInfo.column / effectiveWidth) : 0;
            ...
            const nextValue = currentValue.slice(0, afterDelete) +
                replacement +
                currentValue.slice(currentCursorOffset);
    ...
    const renderVisualRow = useCallback((row, visualIndex) => {
        const isCursorRow = visualIndex === cursorInfo.visualRow;
        if (!isCursorRow || !showCursor || !focus) {
            return _jsx(Text, { children: row.text || " " });
        }
        ...
        return (_jsxs(Text, { children: [before, _jsx(Text, { inverse: cursorVisible, children: cursorChar }), after] }));
    }, [
        cursorInfo.visualRow,
        cursorInfo.visualCol,
        cursorInfo.logicalCol,
        showCursor,
        focus,
        cursorVisible,
    ]);
    if (editorState.value.length === 0 && placeholder) {
        return (_jsx(Box, { ref: boxRef, flexDirection: "row", flexGrow: 1, children: _jsx(Box, { flexGrow: 1, children: _jsx(Text, { dimColor: true, children: showCursor && focus ? (_jsxs(_Fragment, { children: [_jsx(Text, { inverse: cursorVisible, dimColor: !cursorVisible, children: placeholder[0] || " " }), placeholder.slice(1)] })) : (placeholder) }) }) }));
    }
    if (hasScrolling && maxLines) {
        return (_jsxs(Box, { flexDirection: "row", maxHeight: maxLines, width: "100%", children: [_jsx(Box, { ref: boxRef, flexDirection: "column", flexGrow: 1, maxHeight: maxLines, overflow: "scroll", children: visualRows.map((row, i) => (_jsx(Box, { flexShrink: 0, minHeight: 1, children: renderVisualRow(row, i) }, i))) }), showScrollbar && (_jsx(TextInputScrollbar, { scrollRatio: scrollRatio, visibleRatio: visibleRatio, height: maxLines, color: scrollbarColor, trackColor: scrollbarTrackColor }))] }, "scrolling"));
    }
    return (_jsx(Box, { ref: boxRef, flexDirection: "column", flexGrow: 1, width: "100%", children: visualRows.map((row, i) => (_jsx(Box, { flexShrink: 0, minHeight: 1, children: renderVisualRow(row, i) }, i))) }, "non-scrolling"));
}
