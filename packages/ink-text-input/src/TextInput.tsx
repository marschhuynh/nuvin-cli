import { Box, type BoxRef, measureElement, Text } from "@nuvin/ink";
...
function TextInput({
  ...
  const cursorInfo = useMemo(() => {
    ...
    const wrappedRowsBeforeCursor =
      currentLineLen > 0 ? Math.floor(logicalCol / effectiveWidth) : 0;
  ...
  const handleInput = useCallback(
    (input: string, key: InputKey) => {
      ...
      if (key.leftArrow) {
        ...
        const isAtVisualTop =
          lineInfo.lineIndex === 0 && (!effectiveWidth || lineInfo.column < effectiveWidth);
        ...
        const isAtVisualBottom =
          lineInfo.lineIndex === lastLineIndex && currentWrappedRow >= totalWrappedRows - 1;
        ...
        setValueRef.current(
          nextValue,
          nextCursorOffset,
          replacement.length > 1 ? replacement.length : 0,
        );
        return true;
      } else {
        const nextValue =
