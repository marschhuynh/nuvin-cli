...
import { clampOffset, type VimMode } from "./textNavigation.js";
...
export function syncEditorStateFromExternalValue(
  ...
  const nextCursorOffset =
    externalValue === "" ? 0 : clampOffset(externalValue.length, current.cursorOffset, vimMode);
  return {
    ...current,
    value: externalValue,
...
export function useEditorState({ initialValue, vimMode, onChange }: UseEditorStateOptions) {
  ...
  useEffect(() => {
    if (initialValue !== lastExternalValue.current) {
      ...
      const reconciliation = reconcileExternalValue(
        stateRef.current,
        initialValue,
        vimMode,
