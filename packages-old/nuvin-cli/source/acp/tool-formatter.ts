...
export function formatToolCallTitle(
  ...
  switch (toolName) {
    ...
    case "file_new": {
      const filePath = getString(input, "file_path");
      return filePath
        ? `Create file: ${truncate(filePath, 80)}`
        : "Create file";
    }
    case "ls_tool": {
      const targetPath = getString(input, "path");
      return targetPath
        ? `List files: ${truncate(targetPath, 80)}`
        : "List files";
    }
    case "glob_tool": {
      ...
      if (pattern && targetPath)
        return `Find files: ${truncate(pattern, 50)} in ${truncate(
          targetPath,
          30
        )}`;
    ...
    case "grep_tool": {
      ...
      const targetPath = getString(input, "path");
      if (pattern && targetPath)
        return `Search text: ${truncate(pattern, 40)} in ${truncate(
          targetPath,
          30
        )}`;
    ...
    case "lsp": {
      ...
      if (operation && filePath)
        return `Code intel: ${operation} (${truncate(filePath, 50)})`;
